...
export async function getUserPicks(weekData: WeekIdData): Promise<UserGameResponse> {
  try {
...
export async function getPickedGames(weekData: WeekIdData): Promise<AdminGameResponse> {
  try {
    const token = localStorage.getItem('jwt');
